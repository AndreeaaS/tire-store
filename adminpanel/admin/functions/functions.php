...
function get_users($start_position, $goods_for_page){
    $query = "SELECT customer_id, name, second_name, otchestvo, email, name_character
                FROM customers 
                LEFT JOIN `character`
...
function get_roles(){
    $query = "SELECT id_role, name_character FROM `character`";
    $res = mysql_query($query);
...
function add_customer(){
    ...
    if (empty($name)){    $error .= '<li>Не заполнено имя</li>'; }; // дописываем error 
    if (empty($second_name)){    $error .= '<li>Не заполнена фамилия</li>'; }; // дописываем error
    if (empty($otchestvo)){    $error .= '<li>Не заполнено отчество</li>'; }; // дописываем error 
    if (empty($email)){  $error .= '<li>Не заполнен email</li>'; };
    if (empty($password)) {  $error .= '<li>Не заполнен пароль</li>'; };
    ...
    if (empty($error)) {
        ...
        if($row) {
                ...
                $_SESSION['add_customer']['res'] = '<div id="message-red">
                                        <table border="0" width="100%" cellpadding="0" cellspacing="0">
                                        <tr>
                                       <td class="red-left">Ошибка. <a href="">Пользователь с таким email уже зарегистрирован на сайте!</a></td>
                                       <td class="red-right"><a class="close-red"><img src="'.ADMIN_TEMPLATE.'images/table/icon_close_red.gif"   alt="" /></a></td>
                                        </tr>
                                        </table>
                                        </div>';
                $_SESSION['add_customer']['name'] = $name;
                $_SESSION['add_customer']['second_name'] = $second_name;
                $_SESSION['add_customer']['otchestvo'] = $otchestvo;                                
                $_SESSION['add_customer']['password'] = $password;
                return false;
        }else{//если все ок - регистрируем пользователя
        $email = clear($email);
        $name = clear($name);
        $second_name = clear($second_name);
        $otchestvo = clear($otchestvo);                
        $pass = md5($password);
        
        $query = "INSERT INTO customers (name, second_name, otchestvo, email, password, id_role)
                            VALUES ('$name', '$second_name', '$otchestvo', '$email', '$pass', $id_role)";
                $res = mysql_query($query) or die (mysql_error());
                 if (mysql_affected_rows() > 0){
                    // если запись добавлена, т.е. изменилось что-то в БД
                     $_SESSION['answer'] = '<div id="message-green">
                                        <table border="0" width="100%" cellpadding="0" cellspacing="0">
                                        <tr>
                                       <td class="green-left">Пользователь успешно добавлен! </td>
                                       <td class="green-right"><a class="close-green"><img src="'.ADMIN_TEMPLATE.'images/table/icon_close_green.gif"   alt="" /></a></td>
                                        </tr>
                                        </table>
                                        </div>'; 
                    ...
                    $_SESSION['add_customer']['res'] = '<div id="message-red">
                                        <table border="0" width="100%" cellpadding="0" cellspacing="0">
                                        <tr>
                                       <td class="red-left">Ошибка. <a href="">Ошибка при добавлении пользователя!</a></td>
                                       <td class="red-right"><a class="close-red"><img src="'.ADMIN_TEMPLATE.'images/table/icon_close_red.gif"   alt="" /></a></td>
                                        </tr>
                                        </table>
                                        </div>';
                        ...
                        $_SESSION['add_customer']['second_name'] = $second_name;
                        $_SESSION['add_customer']['otchestvo'] = $otchestvo;                                                
                        $_SESSION['add_customer']['email'] = $email;
                        $_SESSION['add_customer']['password'] = $password;
                        return false;
                }
        }
        ...
        $_SESSION['add_customer']['res'] = '<div id="message-red">
                                        <table border="0" width="100%" cellpadding="0" cellspacing="0">
                                        <tr>
                                       <td class="red-left">Ошибка. <a href="">Не заполнены обязательные поля: <ul>'.$error.'</ul></a></td>
                                       <td class="red-right"><a class="close-red"><img src="'.ADMIN_TEMPLATE.'images/table/icon_close_red.gif"   alt="" /></a></td>
                                        </tr>
                                        </table>
                                        </div>';
...
"<div class='error'>Не заполнены обязательные поля: <ul>$error</ul></div>";
};
/* ===Добавление пользователя=== */
